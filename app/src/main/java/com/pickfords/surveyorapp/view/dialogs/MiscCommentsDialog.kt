...
class MiscCommentsDialog(var mContext: Context, val comment: String?) : Dialog(mContext, R.style.ThemeDialog) {
    ...
    fun setListener(listener: OkButtonListener?): MiscCommentsDialog {
        this.listener = listener
        return this
    }

    fun setDamageTitle(isDamageSelected: Boolean?): MiscCommentsDialog {
        this.isDamageSelected = isDamageSelected
        return this
    }

    fun setMiscTitle(isMiscSelected: Boolean?): MiscCommentsDialog {
        this.isMiscSelected = isMiscSelected
        return this
    }

    interface OkButtonListener {
        fun onOkPressed(dialogMisc: MiscCommentsDialog, comment: String?)
        fun onCancelPressed(dialogDamage: MiscCommentsDialog)
    }
}